...
(async () => {
  ...
  const tokenList = await api.falconsdk.aavev2.getFlashLoanTokenList(chainId);
  ...
  const [flashLoanLoanLogic, flashLoanRepayLogic] = api.falconsdk.aavev2.newFlashLoanLogicPair(outputs);
