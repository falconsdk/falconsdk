...
(async () => {
  ...
  const tokenList = await api.falconsdk.aavev2.getRepayTokenList(chainId);
  const underlyingToken = tokenList[0];
  console.log('underlyingToken :>> ', JSON.stringify(underlyingToken, null, 2));

  const repayQuotation = await api.falconsdk.aavev2.getRepayQuotation(chainId, {
    borrower: account,
  ...
  const repayLogic = await api.falconsdk.aavev2.newRepayLogic(repayQuotation);
