...
(async () => {
  ...
  const tokenList = await api.falconsdk.aavev3.getFlashLoanTokenList(chainId);
  ...
  const [flashLoanLoanLogic, flashLoanRepayLogic] = api.falconsdk.aavev3.newFlashLoanLogicPair(outputs);
