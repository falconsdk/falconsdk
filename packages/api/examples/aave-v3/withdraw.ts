...
(async () => {
  ...
  const tokenList = await api.falconsdk.aavev3.getWithdrawTokenList(chainId);
  ...
  const withdrawQuotation = await api.falconsdk.aavev3.getWithdrawQuotation(chainId, {
    input: {
      token: aToken,
      amount: '10',
    },
  ...
  const withdrawLogic = await api.falconsdk.aavev3.newWithdrawLogic(withdrawQuotation);
