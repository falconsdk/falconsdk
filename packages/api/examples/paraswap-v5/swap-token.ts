...
(async () => {
  ...
  const tokenList = await api.falconsdk.paraswapv5.getSwapTokenTokenList(chainId);
  ...
  const swapTokenQuotation = await api.falconsdk.paraswapv5.getSwapTokenQuotation(chainId, {
    input: {
      token: tokenIn,
      amount: '10',
    },
  ...
  const swapTokenLogic = await api.falconsdk.paraswapv5.newSwapTokenLogic(swapTokenQuotation);
