...
(async () => {
  ...
  const tokenList = await api.falconsdk.morphoblue.getFlashLoanTokenList(chainId);
  ...
  const [flashLoanLoanLogic, flashLoanRepayLogic] = api.falconsdk.morphoblue.newFlashLoanLogicPair(outputs);
