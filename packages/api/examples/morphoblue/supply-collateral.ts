...
(async () => {
  ...
  const tokenList = await api.falconsdk.morphoblue.getSupplyCollateralTokenList(chainId);
  const asset = tokenList[marketId][0];
  console.log('asset :>> ', JSON.stringify(asset, null, 2));

  const supplyCollateralLogic = await api.falconsdk.morphoblue.newSupplyCollateralLogic({
    marketId,
    input: {
      token: asset,
      amount: '10',
