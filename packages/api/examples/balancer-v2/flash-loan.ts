...
(async () => {
  ...
  const tokenList = await api.falconsdk.balancerv2.getFlashLoanTokenList(chainId);
  ...
  const [flashLoanLoanLogic, flashLoanRepayLogic] = api.falconsdk.balancerv2.newFlashLoanLogicPair(outputs);
