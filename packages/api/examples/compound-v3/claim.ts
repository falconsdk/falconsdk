...
(async () => {
  ...
  const tokenList = await api.falconsdk.compoundv3.getClaimTokenList(chainId);
  const COMP = tokenList[0];
  console.log('COMP :>> ', JSON.stringify(COMP, null, 2));

  const claimQuotation = await api.falconsdk.compoundv3.getClaimQuotation(chainId, {
    marketId,
    owner: account,
  });
  console.log('claimQuotation :>> ', JSON.stringify(claimQuotation, null, 2));

  const claimLogic = await api.falconsdk.compoundv3.newClaimLogic(claimQuotation);
