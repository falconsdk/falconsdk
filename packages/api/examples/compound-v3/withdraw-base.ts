...
(async () => {
  ...
  const tokenList = await api.falconsdk.compoundv3.getWithdrawBaseTokenList(chainId);
  ...
  const withdrawBaseQuotation = await api.falconsdk.compoundv3.getWithdrawBaseQuotation(chainId, {
    marketId,
    input: {
      token: cToken,
      amount: '10',
  ...
  const withdrawBaseLogic = await api.falconsdk.compoundv3.newWithdrawBaseLogic(withdrawBaseQuotation);
