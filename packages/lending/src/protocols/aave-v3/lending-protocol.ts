...
export class LendingProtocol extends Protocol {
  ...
  private reserves: logics.aavev3.ReserveTokens[] = [];
  ...
  async getTokensForDeposit() {
    if (!this._tokensForDeposit) {
      const tokenList = await apisdk.falconsdk.aavev3.getSupplyTokenList(this.chainId);
  ...
  async getTokensForBorrow() {
    if (!this._tokensForBorrow) {
      const tokenList = await apisdk.falconsdk.aavev3.getBorrowTokenList(this.chainId);
  ...
  newSupplyLogic({ marketId, input }: SupplyParams) {
    return apisdk.falconsdk.aavev3.newSupplyLogic({
      input,
  ...
  newWithdrawLogic({ marketId, output }: WithdrawParams) {
    return apisdk.falconsdk.aavev3.newWithdrawLogic({
  ...
  newBorrowLogic({ output }: BorrowParams) {
    return apisdk.falconsdk.aavev3.newBorrowLogic({
      output,
  ...
  newRepayLogic({ input, account }: RepayParams) {
    return apisdk.falconsdk.aavev3.newRepayLogic({
      input,
      borrower: account,
      interestRateMode: logics.aavev3.InterestRateMode.variable,
    });
